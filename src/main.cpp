...
void computeNormals(unsigned int * indices, unsigned int indexCount, GLfloat * vertices, unsigned int vcount, unsigned vLength, unsigned int normalOffset){
	...
	for (size_t m = 0; m < (vcount / vLength); m++) {
		...
		glm::vec3 norm_avg(vertices[offsetN], vertices[offsetN + 1], vertices[offsetN + 2]);
		norm_avg = glm::normalize(norm_avg);
		vertices[offsetN] = norm_avg.x; vertices[offsetN + 1] = norm_avg.y; vertices[offsetN + 2] = norm_avg.z;
	}
}
...
int main() {
	...
	CreateObjects();
	createShaders();
	GLuint uniformProjection = 0, uniformModel = 0, uniformView = 0, uniformLightPos = 0, 
		unoformLightAmbient = 0, uniformLightColor = 0, uniformDiffuse = 0, uniformViewPos = 0;
	...
	while (!mainWindow.getShouldClose()) {
		...
		lightPtr->useLight(uniformLightColor, uniformLightPos, unoformLightAmbient, uniformDiffuse);
		t1.useTexture();
